...
def process_movement(direction, angle, mouse, player_num, start_position, conn):
    ...
    try:
        if direction != "9":
            Bewegen(player_num, start_position, conn, direction)
...
def senden(conn, start_position):
    ...
    print(f"Neue Position an Client gesendet: {position_data}")

## ------------------------------------------------------------------------
def start_server(local_ip, player_number, local_addr):
    player_addr = []  
    start_position = [[1, 1], [1, 2399], [2399, 1], [2399, 2399]]  
...
player_number = int(input("Wie viele Spieler -> "))  
